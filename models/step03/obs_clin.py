...
@model(
    ...
    columns={
        ...
        "data_partner_id": "int",
        "mapped_obsclin_type": "string",
        "OBSCLIN_START_DATETIME": "timestamp",
        "OBSCLIN_STOP_DATETIME": "timestamp",
    },
)
def entrypoint(context: ExecutionContext, **kwargs):
    ...
    df = create_datetime_col(
        df, "obsclin_start_date", "obsclin_start_time", "OBSCLIN_START_DATETIME"
    )
    df = create_datetime_col(
        df, "obsclin_stop_date", "obsclin_stop_time", "OBSCLIN_STOP_DATETIME"
    )

    mapping_df = read_csv(context.spark, "mapping/mapping.csv")
    df = add_mapped_vocab_code_col(
        df, mapping_df, "OBS_CLIN", "obsclin_type", "mapped_obsclin_type"
    )
