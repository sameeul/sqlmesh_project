...
lab_result_cm as (
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` xw3
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` bloodresult
...
obs_clin as (
    ...
    FROM `{{ ref('pcornet.step03_prepared_obs_clin') }}` obs
        ...
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` xwresult
...
obsgen_lc_num as (
    ...
    SELECT patid AS site_patid, 
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_obs_gen') }}` obg
    INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` unit
...
-- Diastolic BP
    SELECT
        ...
        payload
    FROM `{{ ref('pcornet.step03_prepared_vital') }}` v
        INNER JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` bp 
