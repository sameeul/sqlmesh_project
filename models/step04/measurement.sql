MODEL (
  name pcornet.step04_domain_mapping_measurement,
  kind FULL,
  dialect spark,
  tags ['step04'],
...
lab_result_cm as (
    ...
    LEFT JOIN pcornet.aux_p2o_valueset_mapping_table xw3
    ...
    LEFT JOIN pcornet.aux_p2o_valueset_mapping_table bloodresult
...
obs_clin as (
    ...
    FROM pcornet.step03_prepared_obs_clin obs
        ...
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table xwresult
...
obsgen_lc_num as (
    ...
    SELECT patid AS site_patid, 
        ...
        32817 AS measurement_type_concept_id,
        CAST(null as int) AS operator_concept_id,
        CAST(obg.obsgen_result_num as float) AS value_as_number,
        CAST(null as int) AS value_as_concept_id,
        ...
        encounterid as site_encounterid,
        CAST(null as long) AS visit_detail_id,
        CAST(obg.obsgen_code as string) AS measurement_source_value,
        CAST(xw.source_concept_id as int) AS measurement_source_concept_id,
        CAST(obg.obsgen_result_unit as string) AS unit_source_value,
        CAST(obg.obsgen_result_num as string) AS value_source_value,
        'OBS_GEN' AS domain_source,
        obg.obsgenid as site_pkey,
        data_partner_id,
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_obs_gen obg
    INNER JOIN pcornet.step04_p2o_code_xwalk xw 
    ...
    LEFT JOIN pcornet.aux_p2o_valueset_mapping_table unit
        ON unit.CDM_TBL = 'PRESCRIBING' AND trim(obg.obsgen_result_unit) = unit.SRC_CODE AND unit.CDM_TBL_COLUMN_NAME = 'RX_DOSE_ORDERED_UNIT'
...
procedures as (
    SELECT
        ...
        data_partner_id,
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_procedures pr
        INNER JOIN pcornet.step04_p2o_code_xwalk xw 
...
-- Diastolic BP
    SELECT
        ...
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_vital v
        INNER JOIN pcornet.aux_p2o_valueset_mapping_table bp 
        ...
        FROM (
            SELECT
                ...
                , @stable_hash(concat_ws(
                    ...
                    , COALESCE(CAST(measurement_datetime AS string), '')
