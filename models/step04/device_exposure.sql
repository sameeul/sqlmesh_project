MODEL (
  name pcornet.step04_domain_mapping_device_exposure,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
(
    SELECT 
        ...
        data_partner_id,
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_obs_gen o
    LEFT JOIN pcornet.step04_vent_code_xwalk ventcode_xwalk
    ...
    FROM (
        ...
        , @stable_hash(concat_ws(
              ';'
                ...
                , COALESCE(CAST(device_exposure_start_datetime AS string), '')
                , COALESCE(device_exposure_end_date, '')
                , COALESCE(CAST(device_exposure_end_datetime AS string), '')
