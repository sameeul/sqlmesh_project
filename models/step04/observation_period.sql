...
with obs_period AS (
	...
	FROM (
		...
		FROM `{{ ref('pcornet.step04_domain_mapping_condition_occurrence') }}`
		...
		UNION 
            ...
            FROM `{{ ref('pcornet.step04_domain_mapping_drug_exposure') }}`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
            ...
            FROM `{{ ref('pcornet.step04_domain_mapping_procedure_occurrence') }}`
	        GROUP BY site_patid, data_partner_id, payload
	    UNION
            SELECT 
				  site_patid
            ...
            FROM `{{ ref('pcornet.step04_domain_mapping_observation') }}`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
            ...
            FROM `{{ ref('pcornet.step04_domain_mapping_measurement') }}`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
		    SELECT 
				  site_patid
			...
			FROM `{{ ref('pcornet.step04_domain_mapping_device_exposure') }}`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
			SELECT 
				  site_patid
		...
		UNION 
			...
			FROM `{{ ref('pcornet.step04_domain_mapping_death') }}`
			GROUP BY site_patid, data_partner_id, payload
	) t
