MODEL (
  name pcornet.step04_domain_mapping_observation_period,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
with obs_period AS (
	...
	FROM (
		...
		FROM pcornet.step04_domain_mapping_condition_occurrence
		GROUP BY site_patid, data_partner_id
		UNION 
            SELECT 
				  site_patid
				, data_partner_id
            ...
            FROM pcornet.step04_domain_mapping_drug_exposure
		    GROUP BY site_patid, data_partner_id
		UNION 
            SELECT 
				  site_patid
            ...
            FROM pcornet.step04_domain_mapping_procedure_occurrence
	        GROUP BY site_patid, data_partner_id
	    UNION
            SELECT 
				  site_patid
            ...
            FROM pcornet.step04_domain_mapping_observation
		    GROUP BY site_patid, data_partner_id
		UNION 
            SELECT 
				  site_patid
            ...
            FROM pcornet.step04_domain_mapping_measurement
		    GROUP BY site_patid, data_partner_id
		UNION 
		    SELECT 
				  site_patid
			...
			FROM pcornet.step04_domain_mapping_device_exposure
		    GROUP BY site_patid, data_partner_id
		UNION 
			SELECT 
				  site_patid
			...
			FROM pcornet.step04_domain_mapping_visit_occurrence
			GROUP BY site_patid, data_partner_id
		UNION 
			SELECT
				  site_patid
			...
			FROM pcornet.step04_domain_mapping_death
			GROUP BY site_patid, data_partner_id
	) t
	GROUP BY t.site_patid, t.data_partner_id
)
...
FROM (
    SELECT
    	...
    	, @stable_hash(concat_ws(
			...
			, COALESCE(site_patid, '')
			, COALESCE(min_date, '')
			, COALESCE(CAST(min_datetime AS string), '')
			, COALESCE(max_date, '')
			, COALESCE(CAST(max_datetime AS string), '')
