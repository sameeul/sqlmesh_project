...
with enc_map as (
  SELECT  
    ...
    payload
    FROM (
      SELECT DISTINCT facility_type, data_partner_id, payload FROM `{{ ref('pcornet.step03_prepared_encounter') }}` encounter 
        WHERE encounter.facility_type is not null 
    ) enc
    INNER JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` fx 
