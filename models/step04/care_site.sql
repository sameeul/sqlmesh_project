MODEL (
  name pcornet.step04_domain_mapping_care_site,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
with enc_map as (
  SELECT  
    ...
    FROM (
      SELECT DISTINCT facility_type, data_partner_id, CAST(NULL AS string) AS payload FROM pcornet.step03_prepared_encounter encounter 
        WHERE encounter.facility_type is not null 
    ) enc
    INNER JOIN pcornet.aux_p2o_valueset_mapping_table fx 
