...
with condition as (
    ...
    FROM pcornet.step03_prepared_condition c
        JOIN pcornet.step04_p2o_code_xwalk xw 
...
-- PAT_PREF_LANGUAGE_SPOKEN
    ...
    FROM pcornet.step03_prepared_demographic demo
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table lang 
        ...
        LEFT JOIN (
            ...
            FROM pcornet.step03_prepared_encounter 
            GROUP BY patid
            ) ee 
...
-- GENDER_IDENTITY
    SELECT DISTINCT
        ...
        , CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_demographic demo     
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table gx 
...
-- SEXUAL_ORIENTATION
    SELECT DISTINCT
        ...
        , CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_demographic demo   
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table sx 
...
diagnosis as (
    ...
    FROM pcornet.step03_prepared_diagnosis d
        ...
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table xw2 
...
lab_result_cm as (
    SELECT
        ...
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table u 
...
obsgen_sdoh as (
   ...
   --- COALESCE(xw_map1.TARGET_CONCEPT_ID, xw_map2.TARGET_CONCEPT_ID) AS value_as_concept_id,  --- use xw2 or xw3, this would be the concept id of the categorical data on obsgen_result_text
    ...
    LEFT JOIN pcornet.aux_p2o_valueset_mapping_table obsgensrc
            ...
            AND obsgensrc.CDM_TBL = 'OBS_GEN'
            AND obsgensrc.CDM_TBL_COLUMN_NAME = 'OBSGEN_SOURCE'
    LEFT JOIN pcornet.aux_p2o_valueset_mapping_table modifier
    ...
    LEFT JOIN pcornet.aux_p2o_valueset_mapping_table categorical
...
pro_cm as (
    ...
    FROM pcornet.step03_prepared_pro_cm pro
        ...
        --INNER JOIN pcornet.step04_p2o_code_xwalk xw 
            ...
            --AND xw.CDM_TBL = 'PRO_CM' AND xw.target_domain_id = 'Observation' 
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table prosrc
...
procedures as (
    SELECT
        ...
        CAST(NULL AS string) AS payload
    FROM
        pcornet.step03_prepared_procedures pr
        JOIN pcornet.step04_p2o_code_xwalk xw 
...
-- Smoking
    ...
    FROM pcornet.step03_prepared_vital v
        INNER JOIN pcornet.aux_p2o_valueset_mapping_table s 
            ON s.CDM_TBL = 'VITAL'
            AND s.CDM_TBL_COLUMN_NAME = 'SMOKING'
            AND s.SRC_CODE = v.smoking
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table vt 
            ON vt.CDM_TBL = 'VITAL'
...
-- Tobacco
    SELECT
        ...
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_vital v
        JOIN pcornet.aux_p2o_valueset_mapping_table s 
            ON s.CDM_TBL = 'VITAL'
...
all_other_tables AS (
    SELECT
        ...
        CAST(NULL AS string) AS payload
    FROM (
        SELECT patid, data_partner_id, CAST(NULL AS string) AS payload
        ...
        SELECT 
            patid, data_partner_id, CAST(NULL AS string) AS payload
