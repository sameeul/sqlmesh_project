MODEL (
  name pcornet.step04_domain_mapping_observation,
  kind FULL,
  dialect spark,
  tags ['step04'],
...
with condition as (
    SELECT DISTINCT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_condition') }}` c
        JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
...
-- PAT_PREF_LANGUAGE_SPOKEN
    SELECT DISTINCT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_demographic') }}` demo
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` lang 
        ...
        LEFT JOIN (
            ...
            FROM `{{ ref('pcornet.step03_prepared_encounter') }}` 
            GROUP BY patid
            ) ee 
...
-- GENDER_IDENTITY
    SELECT DISTINCT
        ...
        , payload
    FROM `{{ ref('pcornet.step03_prepared_demographic') }}` demo     
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` gx 
...
-- SEXUAL_ORIENTATION
    SELECT DISTINCT
        ...
        , payload
    FROM `{{ ref('pcornet.step03_prepared_demographic') }}` demo   
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` sx 
...
lab_result_cm as (
    SELECT
        ...
        , data_partner_id
        , payload
        FROM `{{ ref('pcornet.step03_prepared_lab_result_cm') }}` lab
        ...
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` u 
...
obs_clin as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_obs_clin') }}` obs
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
...
obs_gen as ( 
    ...
    CAST(null as string) AS qualifier_source_value,
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_obs_gen') }}` obsg
...
obsgen_sdoh as (
   ...
   --- COALESCE(xw_map1.TARGET_CONCEPT_ID, xw_map2.TARGET_CONCEPT_ID) AS value_as_concept_id,  --- use xw2 or xw3, this would be the concept id of the categorical data on obsgen_result_text
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` obsgensrc
            ON obs.obsgen_source = obsgensrc.SRC_CODE
            AND obsgensrc.CDM_TBL = 'OBS_GEN'
            AND obsgensrc.CDM_TBL_COLUMN_NAME = 'OBSGEN_SOURCE'
    LEFT JOIN `{{ ref('pcornet.step04_answer_code_xwalk') }}`  answercode
   ...
   --- COALESCE(xw_map1.TARGET_CONCEPT_ID, xw_map2.TARGET_CONCEPT_ID) AS value_as_concept_id,  --- use xw2 or xw3, this would be the concept id of the categorical data on obsgen_result_text
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` obsgensrc
            ...
            AND obsgensrc.CDM_TBL = 'OBS_GEN'
            AND obsgensrc.CDM_TBL_COLUMN_NAME = 'OBSGEN_SOURCE'
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` modifier
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` categorical
...
pro_cm as (
    SELECT
        ...
        encounterid as site_encounterid,
        CAST(null as long) AS visit_detail_id,
        CAST(pro.pro_code as string) AS observation_source_value, -- pro_code used to hold pro_code in v7 or (pro_item_loinc & pro_measure_loinc) from v6
        CAST(xw.target_concept_id as int) AS observation_source_concept_id,
        CAST(null as string) AS unit_source_value, -- lookup unit_concept_code
        CAST(null as string) AS qualifier_source_value,
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_pro_cm') }}` pro
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
            ON trim(pro.pro_code) = trim(xw.src_code) -- pro_code used to hold pro_code in v7 or (pro_item_loinc & pro_measure_loinc) from v6
            AND xw.CDM_TBL = 'PRO_CM' AND xw.target_domain_id = 'Observation' 
        --INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
            --ON pro.pro_measure_loinc = xw.src_code
            --AND xw.CDM_TBL = 'PRO_CM' AND xw.target_domain_id = 'Observation' 
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` prosrc
...
procedures as (
    SELECT
        ...
        payload
    FROM
        `{{ ref('pcornet.step03_prepared_procedures') }}` pr
        JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
...
-- Smoking
    SELECT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_vital') }}` v
        INNER JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` s 
            ON s.CDM_TBL = 'VITAL'
            AND s.CDM_TBL_COLUMN_NAME = 'SMOKING'
            AND s.SRC_CODE = v.smoking
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` vt 
            ON vt.CDM_TBL = 'VITAL'
...
-- Tobacco
    SELECT
        ...
        payload
    FROM `{{ ref('pcornet.step03_prepared_vital') }}` v
        JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` s 
            ON s.CDM_TBL = 'VITAL'
...
encounter as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_encounter') }}` d
...
all_other_tables AS (
    ...
    FROM (
        ...
        FROM `{{ ref('pcornet.step03_prepared_death_cause') }}` dc
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
        ...
        FROM `{{ ref('pcornet.step03_prepared_dispensing') }}` d
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
        ...
        FROM `{{ ref('pcornet.step03_prepared_immunization') }}` i
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
        ...
        SELECT patid, data_partner_id, payload
            ...
            , xw.source_concept_id      AS source_concept_id
            , medadminid                AS pkey
        FROM `{{ ref('pcornet.step03_prepared_med_admin') }}` m
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
        ...
        FROM `{{ ref('pcornet.step03_prepared_prescribing') }}` p
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
