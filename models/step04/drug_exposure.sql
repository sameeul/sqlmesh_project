MODEL (
  name pcornet.step04_domain_mapping_drug_exposure,
  kind FULL,
  dialect spark,
  tags ['step04'],
...
med_admin as (
    ...
    FROM `{{ ref('pcornet.step03_prepared_med_admin') }}` m
        ...
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` r 
...
prescribing as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_prescribing') }}` pr
            ...
            LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` mx 
...
immunization as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_immunization') }}` im
        INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` xw 
