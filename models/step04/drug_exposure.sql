...
med_admin as (
    ...
    FROM pcornet.step03_prepared_med_admin m
        ...
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table r 
...
prescribing as (
    SELECT
        ...
        data_partner_id,
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_prescribing pr
            ...
            LEFT JOIN pcornet.aux_p2o_valueset_mapping_table mx 
...
immunization as (
    SELECT
        ...
        data_partner_id,
        CAST(NULL AS string) AS payload
    FROM pcornet.step03_prepared_immunization im
        INNER JOIN pcornet.step04_p2o_code_xwalk xw 
...
FROM (
    ...
    , @stable_hash(concat_ws(
            ...
            , COALESCE(CAST(drug_exposure_start_datetime AS string), '')
            , COALESCE(drug_exposure_end_date, '')
            , COALESCE(CAST(drug_exposure_end_datetime AS string), '')
