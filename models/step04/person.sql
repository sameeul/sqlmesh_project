MODEL (
  name pcornet.step04_domain_mapping_person,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
with patients_with_records AS (
    SELECT DISTINCT patid FROM (
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_condition') }}`		
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_death') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_death_cause') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_diagnosis') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_dispensing') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_encounter') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_immunization') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_lab_result_cm') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_med_admin') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_obs_clin') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_obs_gen') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_prescribing') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_pro_cm') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_procedures') }}`
            UNION ALL
        SELECT DISTINCT patid FROM `{{ ref('pcornet.step03_prepared_vital') }}`
    )
),
...
demographic as (
    ...
    FROM `{{ ref('pcornet.step03_prepared_demographic') }}` demo  
        ...
        LEFT JOIN `{{ ref('pcornet.aux_ethnicity_xwalk_table') }}` ex 
            ON ex.CDM_TBL = 'DEMOGRAPHIC' 
            AND demo.hispanic = ex.SRC_ETHNICITY 
        LEFT JOIN `{{ ref('pcornet.aux_race_xwalk_table') }}` rx 
