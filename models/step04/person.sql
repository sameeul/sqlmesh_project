...
with patients_with_records AS (
    SELECT DISTINCT patid FROM (
        SELECT DISTINCT patid FROM pcornet.step03_prepared_condition		
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_death
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_death_cause
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_diagnosis
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_dispensing
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_encounter
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_immunization
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_lab_result_cm
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_med_admin
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_obs_clin
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_obs_gen
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_prescribing
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_pro_cm
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_procedures
            UNION ALL
        SELECT DISTINCT patid FROM pcornet.step03_prepared_vital
    )
),
...
demographic as (
    ...
    FROM pcornet.step03_prepared_demographic demo  
        ...
        LEFT JOIN pcornet.aux_gender_xwalk_table gx 
            ON gx.CDM_TBL = 'DEMOGRAPHIC'
            AND demo.sex = gx.SRC_GENDER 
        LEFT JOIN pcornet.aux_ethnicity_xwalk_table ex 
            ON ex.CDM_TBL = 'DEMOGRAPHIC' 
            AND demo.hispanic = ex.SRC_ETHNICITY 
        LEFT JOIN pcornet.aux_race_xwalk_table rx 
    ...
    FROM (
        ...
        FROM (
            ...
            FROM (
                ...
                , md5(CAST(site_patid AS string)) as hashed_id
                FROM demographic
            )
        )
    )
