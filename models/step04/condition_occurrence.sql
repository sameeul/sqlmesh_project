MODEL (
  name pcornet.step04_domain_mapping_condition_occurrence,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
with cond_diagnosis_obsclin AS (
    ...
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` csrc 
        ON csrc.CDM_TBL = 'CONDITION' 
        AND csrc.CDM_TBL_COLUMN_NAME = 'CONDITION_SOURCE' 
        AND csrc.SRC_CODE = c.condition_source
    LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` cst 
