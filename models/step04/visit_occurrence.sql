MODEL (
  name pcornet.step04_domain_mapping_visit_occurrence,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
-- encounter
    ...
    FROM pcornet.step03_prepared_encounter enc
        LEFT JOIN pcornet.aux_visit_xwalk_mapping_table vx
            ON vx.CDM_TBL = 'ENCOUNTER' 
            AND vx.CDM_NAME = 'PCORnet' 
            AND trim(vx.SRC_VISIT_TYPE) = trim(enc.enc_type)
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table vsrc 
        ...
        LEFT JOIN pcornet.aux_p2o_valueset_mapping_table disp 
    ...
    FROM (
        ...
        FROM (
            ...
            FROM (
                ...
                , md5(CAST(site_encounterid AS string)) as hashed_id
