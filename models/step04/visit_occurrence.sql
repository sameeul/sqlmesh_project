...
-- encounter
    SELECT distinct
        ...
        data_partner_id,
        payload
    FROM `{{ ref('pcornet.step03_prepared_encounter') }}` enc
        LEFT JOIN `{{ ref('pcornet.aux_visit_xwalk_mapping_table') }}` vx
        ...
        LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` disp 
