...
with death_cause_ext as (
  ...
  INNER JOIN pcornet.step04_p2o_code_xwalk cs 
...
FROM pcornet.step03_prepared_death death
LEFT JOIN death_cause_ext dc
  ON dc.patid = death.patid
LEFT JOIN pcornet.aux_p2o_valueset_mapping_table dt 
  ON dt.CDM_TBL = 'DEATH' 
...
FROM
    ...
    FROM
        pcornet.step03_prepared_encounter d
      LEFT JOIN pcornet.step03_prepared_death dc 
        ON dc.patid = d.patid
      WHERE
          discharge_status = 'EX'
