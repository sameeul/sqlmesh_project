MODEL (
  name pcornet.step04_domain_mapping_death,
  kind FULL,
  dialect spark,
  tags ['step04'],
);
...
with death_cause_ext as (
  ...
  INNER JOIN `{{ ref('pcornet.step04_p2o_code_xwalk') }}` cs 
...
SELECT
   ...
   , death.payload
FROM `{{ ref('pcornet.step03_prepared_death') }}` death
LEFT JOIN death_cause_ext dc
  ON dc.patid = death.patid
LEFT JOIN `{{ ref('pcornet.aux_p2o_valueset_mapping_table') }}` dt 
  ON dt.CDM_TBL = 'DEATH' 
...
FROM
    ...
    FROM
        `{{ ref('pcornet.step03_prepared_encounter') }}` d
      LEFT JOIN `{{ ref('pcornet.step03_prepared_death') }}` dc 
        ON dc.patid = d.patid
      WHERE
          discharge_status = 'EX'
