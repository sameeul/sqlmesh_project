...
(
	SELECT DISTINCT
		...
		c.concept_id
	FROM `{{ ref('pcornet.aux_concept_ancestor') }}` ca
		INNER JOIN `{{ ref('pcornet.aux_concept') }}` c ON ca.ancestor_concept_id = c.concept_id
...
(-- Normalize DRUG_EXPOSURE_END_DATE to either the existing drug exposure end date, or add days supply, or add 1 day to the start date
	SELECT
		...
		, data_partner_id
		, payload
	FROM `{{ ref('pcornet.step04_domain_mapping_drug_exposure') }}` d
