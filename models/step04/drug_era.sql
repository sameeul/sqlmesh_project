MODEL (
  name pcornet.step04_domain_mapping_drug_era,
  kind FULL,
  dialect spark,
  tags ['step04'],
...
(
	SELECT DISTINCT
		...
		c.concept_id
	FROM pcornet.aux_concept_ancestor ca
		INNER JOIN pcornet.aux_concept c ON ca.ancestor_concept_id = c.concept_id
...
(-- Normalize DRUG_EXPOSURE_END_DATE to either the existing drug exposure end date, or add days supply, or add 1 day to the start date
	...
	FROM pcornet.step04_domain_mapping_drug_exposure d
...
(
	...
	GROUP BY person_id, drug_concept_id, drug_sub_exposure_end_date, data_partner_id
...
GROUP BY person_id, drug_concept_id, drug_era_end_date, data_partner_id
ORDER BY person_id, drug_concept_id
)
