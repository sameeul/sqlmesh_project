MODEL (
  name pcornet.step04_p2o_code_xwalk,
  kind FULL,
  dialect spark,
  tags ['step04_helpers'],
);
...
WITH site_codes_to_source_concept_mapping AS (
    ...
    SELECT DISTINCT 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_procedures') }}` p
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
    ...
    SELECT DISTINCT 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_condition') }}` cd
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
    ...
    SELECT DISTINCT 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_death_cause') }}`dt
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
    ...
    SELECT DISTINCT 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_lab_result_cm') }}` l
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
    ...
    SELECT DISTINCT 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_dispensing') }}` dx
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
    ...
    SELECT DISTINCT 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_obs_clin') }}` o
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
    ...
    SELECT DISTINCT  
        'PRO_CM' as CDM_TBL
        ,pro.pro_code as src_code -- pro_code used to hold pro_code in v7 or (pro_item_loinc & pro_measure_loinc) from v6
        ...
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
            ON c.concept_code = pro.pro_code AND (upper(c.vocabulary_id) = upper('LOINC')) -- pro_code used to hold pro_code in v7 or (pro_item_loinc & pro_measure_loinc) from v6
            AND c.concept_class_id != 'ICD10PCS Hierarchy' -- codes overlap with ICD10CM
        where pro.pro_type = 'LC'
    --UNION ALL      ##commenting this block out bc pro_measure_loinc is deprecated and also replaced by the same pro_code above
    --SELECT DISTINCT  
    --    'PRO_CM' as CDM_TBL
    ...
    select 
        ...
        , c.valid_end_date              AS source_valid_end_date
        , c.invalid_reason              AS source_invalid_reason
        FROM `{{ ref('pcornet.step03_prepared_obs_gen') }}` ob
        LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c
...
target_concept_mapping AS (
    ...
    LEFT JOIN `{{ ref('pcornet.aux_concept_relationship') }}` cr 
        ON site_codes_to_source_concept_mapping.source_concept_id = cr.concept_id_1
        AND cr.invalid_reason IS NULL 
        AND lower(cr.relationship_id) = 'maps to'
    LEFT JOIN `{{ ref('pcornet.aux_concept') }}` c2
